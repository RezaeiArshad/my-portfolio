import {dotsSvg, arrowSvg} from '../assets/titleSvg'
import { useScreenSize } from '../contexts';
import { motion } from 'motion/react';
import { useAnimationFrame } from "motion/react"
import { useRef } from "react";


function UseAnimationFrame() {
  const ref = useRef(null)
  useAnimationFrame((t) => {
      if (!ref.current) return
      const rotate = Math.sin(t / 10000) * 200
      const y = (1 + Math.sin(t / 1000)) * -50
      ref.current.style.transform = `translateY(${y}px) rotateX(${rotate}deg) rotateY(${rotate}deg)`
  })
  return (
      <motion.div 
      initial={{opacity: 0, x: 200}}
      whileInView={{opacity: 1, x: 0}}
      viewport={{ once: false, margin: "+10px"}}
      transition={{duration: 1}}
      className="container col-12">
          <div className="cube" ref={ref}>
              <div className="side front" />
              <div className="side left" />
              <div className="side right" />
              <div className="side top" />
              <div className="side bottom"></div>
              <div className="side middle"><svg fill="none" strokeLinecap="square" strokeMiterlimit="10" version="1.1" viewBox="0 0 226.77 226.77" xmlns="http://www.w3.org/2000/svg">
                <g transform="translate(8.964 4.2527)" fillRule="evenodd" stroke="" strokeLinecap="butt" strokeLinejoin="round" strokeWidth="4">
                <path d="m63.02 200.61-43.213-174.94 173.23 49.874z"/>
                <path d="m106.39 50.612 21.591 87.496-86.567-24.945z"/>
                <path d="m84.91 125.03-10.724-43.465 43.008 12.346z"/>
                <path d="m63.458 38.153 10.724 43.465-43.008-12.346z"/>
                <path d="m149.47 62.93 10.724 43.465-43.008-12.346z"/>
                <path d="m84.915 125.06 10.724 43.465-43.008-12.346z"/>
                </g>
               </svg></div>
              <div className="side back" />
          </div>
      </motion.div>
  )
}

function TitlePart() {
  const { isMobile } = useScreenSize();
  return (
    <>
      {isMobile ? (
        <div className="row">
          {/* this is for mobile */}
          <img
            className="profile-pic d-block mx-auto img-fluid fadein col-6"
            src="\page-pictures\dark-theme\about-me\altumcode-U0tBTn8UR8I-unsplash.jpg"
          />
          <h1>Hi</h1>
          <h1>I'm Mehdi</h1>
          <button>plus</button>
        </div>
      ) : (
        <div>
        <div id="title-name" style={{ width: "100%" }}>
          {/* this for laptop */}
          <div className="row mb-0" style={{ width: "100%" }}>
          <div style={{ textAlign: "left" }} className="mt-auto mb-auto offset-md-2 p-0 col-7">
            <motion.h3
              initial={{opacity: 0}}
              animate={{opacity: 1}}
              transition={{duration: 1 , ease: "easeIn"}}
              style={{fontWeight: "bold"}} className="highlight mb-0 p-0 h3-clamp">Hello there, I'm -</motion.h3>
            {dotsSvg}
            <motion.h1
              initial={{opacity: 0, x: -50}}
              animate={{opacity: 1, x: 0}}
              transition={{duration: 1}}
              id="title-name-h1"
              className="h1-clamp p-0 m-0 fade-in me-auto col-12">Mehdi Rezaei</motion.h1>
            <h4 id="title-name-h4" className="col-7 p-0 me-auto">Software Developer: <span className="forground">currently specializing in Frontend development.</span></h4>
            <motion.h5
              initial={{opacity: 0}}
              animate={{opacity: 1}}
              transition={{duration: 1}}
              id="title-name-h5-1">🚀 Self-Taught React.js Programmer</motion.h5>
            <h5 id="title-name-h5-2"><motion.span
              initial={{scale: 1, opacity: 1}}
              animate={{scale: 1.5, opacity: 0}}
              transition={{duration: 1, repeat: Infinity, repeatType: "reverse"}}
              className="green-span"
            ></motion.span><motion.span
              initial={{scale: 1, opacity: 1}}
              animate={{scale: 1.5, opacity: 0}}
              transition={{duration: 1, repeat: Infinity, repeatType: "reverse", delay: 0.5}}
              className="green-span"
            ></motion.span>looking for an internship</h5>
          </div>
          <div id='title-name-button-div' className="col-2">
            <a href='https://github.com/rezaeiarshad' target='_blank'>
            <motion.button
              initial={{y: 50, opacity: 0}}
              animate={{y: 0, opacity: 1}}
              transition={{ duration: 1}}
              className="d-block col-10 mt-auto"><svg id="github-icon" stroke="currentColor" strokeWidth="0" viewBox="0 0 496 512" color="#3CCF91" aria-hidden="true" focusable="false" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg" ><path d="M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3.3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5.3-6.2 2.3zm44.2-1.7c-2.9.7-4.9 2.6-4.6 4.9.3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3.7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3.3 2.9 2.3 3.9 1.6 1 3.6.7 4.3-.7.7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3.7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3.7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z"></path></svg> Github</motion.button>
            </a>
            <a href='https://www.linkedin.com/in/mehdi-rezaeiarshad-b33975326?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app' target='_blank'>
            <motion.button
              initial={{y: 60, opacity: 0}}
              animate={{y: 0, opacity: 1}}
              transition={{ duration: 1}}
              className="d-block col-10"><svg id="linkedin-icon" stroke="currentColor" strokeWidth="0" viewBox="0 0 448 512" color="#3CCF91" aria-hidden="true" focusable="false" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M416 32H31.9C14.3 32 0 46.5 0 64.3v383.4C0 465.5 14.3 480 31.9 480H416c17.6 0 32-14.5 32-32.3V64.3c0-17.8-14.4-32.3-32-32.3zM135.4 416H69V202.2h66.5V416zm-33.2-243c-21.3 0-38.5-17.3-38.5-38.5S80.9 96 102.2 96c21.2 0 38.5 17.3 38.5 38.5 0 21.3-17.2 38.5-38.5 38.5zm282.1 243h-66.4V312c0-24.8-.5-56.7-34.5-56.7-34.6 0-39.9 27-39.9 54.9V416h-66.4V202.2h63.7v29.2h.9c8.9-16.8 30.6-34.5 62.9-34.5 67.2 0 79.7 44.3 79.7 101.9V416z"></path></svg> LinkedIn</motion.button>
            </a>
            <a href="mailto:mehdirezaeiarshad@gmail.com" target='_blank'>
            <motion.button
              initial={{y: 70, opacity: 0}}
              animate={{y: 0, opacity: 1}}
              transition={{ duration: 1}}
              className="d-block col-10"><svg id="email-icon"  stroke="currentColor"  strokeWidth="0" viewBox="0 0 512 512" aria-hidden="true" focusable="false" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M502.3 190.8c3.9-3.1 9.7-.2 9.7 4.7V400c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V195.6c0-5 5.7-7.8 9.7-4.7 22.4 17.4 52.1 39.5 154.1 113.6 21.1 15.4 56.7 47.8 92.2 47.6 35.7.3 72-32.8 92.3-47.6 102-74.1 131.6-96.3 154-113.7zM256 320c23.2.4 56.6-29.2 73.4-41.4 132.7-96.3 142.8-104.7 173.4-128.7 5.8-4.5 9.2-11.5 9.2-18.9v-19c0-26.5-21.5-48-48-48H48C21.5 64 0 85.5 0 112v19c0 7.4 3.4 14.3 9.2 18.9 30.6 23.9 40.7 32.4 173.4 128.7 16.8 12.2 50.2 41.8 73.4 41.4z"></path></svg> Email</motion.button>
            </a>
          </div>
          </div>
            {arrowSvg}
        </div>
          <div id="title-working">
            <div className="row" style={{width: "100%"}}>
              <div className="col-3 offset-md-2">
              <motion.img 
              initial={{opacity: 0}}
              whileInView={{opacity: 1}}
              viewport={{once: false, margin: "-60px"}}
              transition={{ duration: 3}}
              src='\page-pictures\dark-theme\about-me\msg-10011895964-450.jpg' />              
              </div>
             <div className="col-6">
               <motion.h1
               initial={{opacity: 0}}
               whileInView={{opacity: 1}}
               viewport={{once: false, margin: "-60px"}}
               className="h2-clamp mb-0 mt-5"
               transition={{ duration: 2}}
               >working <span className="highlight">overTime</span></motion.h1>
               <motion.h3
               className="forground"
               >is becoming a hobby 🔥</motion.h3>
               <motion.h4
               initial={{opacity: 0, x: 200}}
               whileInView={{opacity: 1, x: 0}}
               viewport={{once: false, margin: "-60px"}}
               transition={{duration: 1}}
               className="mt-4 col-8"
               >joy, excitement and fascination have all played hand in hand into keeping me going for more hours</motion.h4>
             </div>
            </div>
           </div>
        <div id='title-improving'>
          <div className="row" style={{width: "100%"}}>
            <div className='offset-md-2 col-5'>
              <motion.h1 
              initial={{opacity: 0}}
              whileInView={{opacity: 1}}
              viewport={{once: false, margin: "+50px"}}
              transition={{ duration: 1}}
              className='col-12 p-0 h2-clamp'>Ever-improving</motion.h1>
              <motion.h4
              initial={{opacity: 0, x: 200}}
              whileInView={{opacity: 1, x: 0}}
              viewport={{once: false, margin: "+50px"}}
              transition={{ duration: 1}}
              className="col-9 forground"
              >I know <span className="text-color">technology moves fast</span> but I'm <span className="text-color">faster</span> , for my next ambition I'll learn <span className="highlight">Three.js</span>. . .</motion.h4>
            </div>
            <div className="col-3">
              <UseAnimationFrame />
            </div>
          </div>
        </div>
        </div>
      )}
    </>
  );
}

export default TitlePart